...
data$bird_id_date<-paste(data$bird_ID,data$date, sep="_")
#make new column with combination of key and bird_played
data$keys_and_songs<-paste(data$key_pecked,data$bird_played, sep="_")
...
for (bid in unique(data$bird_id_date))
{
  bid_rows<-subset(data,bird_id_date==bid)
